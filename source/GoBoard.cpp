...
int GoBoard::select_and_play(int color)
{
	...
	if (random_number < 10000 * PROB_LAST_ATARI_HEURISTIC)
	{
		move = last_atari_heuristic(color);					//try to find a move that will capture the opponent
		if (move != -1)
		{
	...
	random_number = rand() * 10000 / (RAND_MAX + 1);
	if (random_number < 10000 * PROB_CAPTURE_HEURISTIC )
	{
	...
	random_number = rand() * 10000 / (RAND_MAX + 1);
	if (random_number < 10000 * PROB_SAVE_HEURISTIC)
	{
	...
	random_number = rand() * 10000 / (RAND_MAX + 1);
	if (random_number < 10000 * PROB_NAKADE_HEURISTIC)
	{
	...
	random_number = rand() * 10000 / (RAND_MAX + 1);
	if (random_number < 10000 * PROB_PATTERN_HEURISTIC)
	{
